...
const SETTLED_EVENT = parseAbiItem(
  "event Settled(uint256 indexed seriesId, uint256 settlePrice)" // adjust if signature differs
);
...
export default function TakerPortfolio() {
  ...
  // Load series from historical logs once
  useEffect(() => {
    ...
    const fetchSeries = async () => {
      ...
      try {
        ...
        const step = 20_000n;
        const accum: SeriesData[] = [];

        for (let b = fromBlock; b <= latestBlock; b += step + 1n) {
          const toBlock = b + step > latestBlock ? latestBlock : b + step;
          ...
          for (const l of logs) {
            // typed destructure to avoid readonly tuple issues
        ...
        accum.forEach((s) => map.set(s.id.toString(), s));
  ...
  // Subscribe to new series definitions
  useWatchContractEvent({
    address: VAULT_ADDRESS,
    abi: vaultAbi,
  ...
  // Load portfolio positions when prerequisites ready
  useEffect(() => {
    if (!address || !publicClient || !allSeries.length || loadingSeries) return;
    ...
    const loadPositions = async () => {
      ...
      try {
        ...
        for (const series of allSeries) {
          ...
          // Fetch series settlement metadata
          let isSettled = false;
          let settlePrice = 0n;

          try {
            const seriesRaw = (await publicClient.readContract({
              address: VAULT_ADDRESS,
              abi: vaultAbi,
              functionName: "series",
              args: [series.id],
            })) as any[];
            // assuming index 6 is settled flag per previous code
            isSettled = Boolean(seriesRaw[6]);
          } catch {
            // swallow; leave as defaults
          }
        ...
        pos.sort((a, b) => Number(a.series.expiry - b.series.expiry));
        setPositions(pos);
      } catch (err) {
  ...
  // Update positions when series get settled
  useWatchContractEvent({
    address: VAULT_ADDRESS,
    abi: vaultAbi,
    eventName: "Settled",
    onLogs: () => {
      // Refresh only positions instead of full reload
      if (address && allSeries.length && publicClient) {
        // Re-trigger position loading by toggling a small state or reusing effect dependencies
        // Simplest: re-run loadPositions via incremental dummy state (here we just call the effect's logic again)
        // A lightweight way is to force re-evaluation by updating allSeries reference (clone)
      ...
      {loadingSeries || loadingPositions ? (
        ...
        <div className="text-center py-8 text-default-500">
          ...
          <div className="text-sm">
            Purchase options from the Orderbook section to see them here
          </div>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          {positions.map((position) => {
            ...
            const moneyness = isSettled
              ? calculateMoneyness(settlePrice, series.strike)
              : null;
            const timeToExpiry =
              Number(series.expiry) - Math.floor(Date.now() / 1000);
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <details className="cursor-pointer">
                      <summary className="text-xs text-default-500">
                        Details
                      </summary>
                      <div className="mt-2 p-3 bg-default-100 rounded text-xs space-y-1">
                        ...
                        <div>
                          <strong>Underlying:</strong> {series.underlying}
                        </div>
                        <div>
                        ...
                        {isSettled && (
                          <>
                            <div>
                              <strong>Settle Price:</strong>{" "}
                              {formatUnits(settlePrice, 18)} WXDAI
                            </div>
                            <div>
                              <strong>Per Option Value:</strong>{" "}
                              {formatUnits(intrinsicValue, 18)} WXDAI
                            </div>
                          </>
                        )}
                      </div>
                ...
                {!isExpired && (
                  <div className="space-y-1">
                    ...
                    <Progress
                      value={Math.min(
                        100,
                        Math.max(0, (daysToExpiry / 30) * 100)
                      )}
                      size="sm"
                      color={
                        daysToExpiry > 7
        ...
        <div className="pt-4 border-t border-default-200">
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              onPress={() => {
                document
                  .querySelector('[data-section="exercise"]')
            ...
            <Button
              ...
              variant="flat"
              onPress={() => {
                // force re-evaluation by nudging series (cheap)
                setAllSeries((prev) => [...prev]);
              }}
            >
              Refresh Positions
